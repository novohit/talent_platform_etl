...
# df_school = pd.read_csv("data/school_info.csv")
...
async def brightdata_api(query: str) -> List[Dict[str, Any]]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(endpoint, headers=headers, json=payload, timeout=90) as response:
...
async def process_row(row) -> pd.DataFrame:
    async with semaphore:
        ...
        df = pd.DataFrame(res, columns=["id", "teacher_id", "raw_data_id", "teacher_name", "school_name", "origin_description", "title", "description", 'html_info', "link", "rank"])
        return df
...
@retry(stop=stop_after_attempt(5), wait=wait_fixed(2), retry_error_callback=return_empty_string_on_retry_error)
