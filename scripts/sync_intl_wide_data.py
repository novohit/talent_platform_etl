...
def safe_json_loads(json_str, field_name, teacher_id, to_camel=False):
    ...
    try:
        ...
        logger.warning(f"Invalid JSON format in field '{field_name}' for teacher_id={teacher_id}, value: {json_str}")
        return None
...
def process_batch(items: List[TeacherWide]):
    documents = []
    for item in items:
        educations = safe_json_loads(item.educations, "educations", item.teacher_id, to_camel=True)
        ...
        employments = safe_json_loads(item.employments, "employments", item.teacher_id, to_camel=True)
        ...
        documents.append(
            {
                ...
                "_source": {
                    ...
                    "researchArea": safe_json_loads(item.research_area, "research_area", item.teacher_id),
                    "normalizedTitle": item.normalized_title,
                    "isPhd": item.is_phd,
                    "isChinese": item.is_chinese,
                    "isDomeCooperation": item.is_dome_cooperation,
                    "famousTitles": safe_json_loads(item.famous_titles, "famous_titles", item.teacher_id),
                    "firstLevel": item.first_level,
                    "secondLevel": item.second_level,
                    "region": item.region,
                    "ranking": item.ranking,
                    "fakeRanking": item.ranking,
                    "paperNum": item.paper_num,
                    "majorPaper1Domain": safe_json_loads(item.major_paper_1_domain, "major_paper_1_domain", item.teacher_id),
                    "minorPaper1Domain": safe_json_loads(item.minor_paper_1_domain, "minor_paper_1_domain", item.teacher_id),
                    "majorPaper2Domain": safe_json_loads(item.major_paper_2_domain, "major_paper_2_domain", item.teacher_id),
                    "minorPaper2Domain": safe_json_loads(item.minor_paper_2_domain, "minor_paper_2_domain", item.teacher_id),
                    "majorPaper3Domain": safe_json_loads(item.major_paper_3_domain, "major_paper_3_domain", item.teacher_id),
                    "ageRange": item.age_range,
                    "corporateExperience": item.corporate_experience,
                    "overseasExperience": item.overseas_experience,
                    "paperl2Domains": safe_json_loads(item.paper_l2_domains, "paper_l2_domains", item.teacher_id),
                    "paperl3Domains": safe_json_loads(item.paper_l3_domains, "paper_l3_domains", item.teacher_id),
...
if __name__ == "__main__":
    ...
    for i in range(0, total_records, batch_size):
        ...
        logger.info(f"Processing batch {i//batch_size + 1}, records {i+1} to {min(i+batch_size, total_records)}")
        process_batch(batch)
